...
# Datum des letzten Ausführens: 31.07.2024
...
if [ -f "$DATEI" ]; then
  # Lese das Datum des letzten Ausführens, ignoriere alle Zeilen außer der ersten
  letztesDatum=$(sed -n '2p' "$DATEI" | grep -oP '\d{2}\.\d{2}\.\d{4}')

  if [ -n "$letztesDatum" ]; then
    # Konvertiere das gelesene Datum von DD.MM.YYYY in YYYY-MM-DD
    formatiertesDatum=$(echo $letztesDatum | awk -F"." '{printf "%04d-%02d-%02d", $3, $2, $1}')

    # Konvertiere das formatierte Datum in Sekunden seit dem 01.01.1970
    sekLetztesDatum=$(date -d "$formatiertesDatum" +%s)
    # Aktuelles Datum in Sekunden seit dem 01.01.1970
    sekHeute=$(date +%s)
    # Berechne die Differenz in Tagen
    diffTage=$(( (sekHeute - sekLetztesDatum) / 86400 ))

    # Ausgabe der Differenz und des Datums im gewünschten Format
...
# Benutze 'sed', um die zweite Zeile zu ersetzen
...
if [[ ! $clean_decision =~ ^[Yy]$ ]]; then
  echo "Reinigung der Bash-Historie abgebrochen."
  exit 0
fi

# Fragt den Benutzer nach einem Kennwort, das aus der Bash-History entfernt werden soll
password=$(kdialog --password "Enter the keyword, that should be removed from history")
# Prüft, ob das Kennwort leer ist oder ein gültiger Befehl
...
# Prüft, ob das Kennwort in der History gefunden wurde
if [[ ! "$removed" =~ [^[:space:]] ]]; then
  ...
  # Fragt den Benutzer, ob die gefundene Befehle wirklich gelöscht werden sollen
...
# Überprüft die Benutzerentscheidung
...
# Aufräumen, falls das temporäre File nicht verschoben wurde
...
# Pfad zur Bash-History-Datei
histfile="$HOME/.bash_history"
# Temporäre Datei für Duplikat-Entfernung erstellen
tmpfile_dup=$(mktemp)
...
# Funktion, um zu prüfen, ob ein Befehl existiert oder ein Alias ist
check_command() {
  type "$1" &>/dev/null
}

# Temporäre Datei erstellen, um gültige Befehle zu speichern
temp_file=$(mktemp)
# Temporäre Datei für entfernte Befehle
removed_commands=$(mktemp)

# Anzahl der Befehle in der Historie vor der gesamten Bereinigung
commands_before=$(wc -l < "$histfile")

# Schleife durch jeden Befehl in der Historie
while IFS= read -r command; do
  # Prüfen, ob der Befehl leer ist oder nur aus Leerzeichen besteht
  if [[ ! $command =~ ^[[:space:]]*$ ]]; then
    # Extrahiere den ersten Befehlsnamen/Programmnamen vor dem ersten Leerzeichen
    cmd=$(echo $command | cut -d ' ' -f1)

    # Behandle Fälle, in denen 'sudo' verwendet wird
    if [[ $cmd == "sudo" ]]; then
      cmd=$(echo $command | cut -d ' ' -f2)
    fi

    # Prüfe, ob der extrahierte Befehl ausführbar ist, ein Alias oder sonstiges
    if check_command "$cmd"; then
      # Gültigen Befehl zur temporären Datei hinzufügen
      echo "$command" >> "$temp_file"
    else
      # Füge entfernten Befehl zur Liste hinzu
...
# Entscheidung, ob entfernte Befehle angezeigt werden sollen
if [[ -s $removed_commands ]]; then
  echo -ne "\e[1;34m>>\e[0m Möchtest du die Befehle sehen, die entfernt werden würden? (y/N): "
  read show_decision
  # Korrigiere die Zuweisung, entferne Leerzeichen um =
  show_decision=${show_decision:-n}
 # Füge ein Leerzeichen vor dem schließenden ']' hinzu
  ...
  read remove_decision
  remove_decision=${remove_decision:-y} # Setze Standardantwort auf 'y'

  # Korrigiere diesen Abschnitt, um die Befehle nur bei Zustimmung zu entfernen
  if [[ $remove_decision =~ ^[Yy]$ ]]; then
    # Wenn der Benutzer zustimmt, überschreibe die Historie-Datei mit der bereinigten Liste
    mv "$temp_file" "$histfile"
    echo "Befehle wurden entfernt."
  else
    echo "Befehle wurden NICHT entfernt."
    # Wenn Befehle nicht entfernt werden, stelle sicher, dass die Original-Historie unverändert bleibt
    # Dies könnte durch Löschen der temporären Datei und nicht durch ihr Verschieben geschehen
    rm "$temp_file"
  fi
else
...
# Überprüfen, ob sich die Anzahl der Befehle geändert hat
if [ "$commands_before" -ne "$commands_after" ]; then
    # Wenn die Anzahl sich geändert hat, Ausgabe der neuen Anzahl
    echo "Anzahl der Befehle in der Historie von $commands_before Befehlen auf $commands_after reduziert."
else
    # Wenn keine Änderung, Ausgabe, dass nichts entfernt wurde
    echo "Es wurden keine Befehle entfernt, die Anzahl der Befehle in der history ist immernoch $commands_before."
fi
...
if [[ elapsedDays -le keepDays ]]; then
    ...
    echo "Systemaktualisierung übersprungen."
fi
...
if [[ $antwort == "y" || $antwort == "Y" ]]; then
    # Temporäres Verzeichnis für das Repository erstellen, Output unterdrücken
    temp_dir=$(mktemp -d 2>/dev/null)

    # Warte auf den YubiKey
    echo "Bitte stecken Sie Ihren YubiKey ein."
    while [ $(lsusb | grep -c 'Yubico') -eq 0 ]; do
        echo -ne "Warten auf YubiKey...\r"
        sleep 0.7  # Wartezeit vor dem "Auslöschen" des Texts
        echo -ne "                     \r"  # Leerzeichen, um den Text zu "löschen"
        sleep 0.7  # Wartezeit vor dem Wiederanzeigen des Texts
    done
    echo "YubiKey erkannt."

    # Klone das Repository ins temporäre Verzeichnis, unterdrücke Ausgabe
    if git clone $repo_url $temp_dir >/dev/null 2>&1; then
        # Wechsle in das temporäre Verzeichnis
        cp -r ~/.mozilla/firefox/*.default/sessionstore-backups/* ~/setup/firefox/
        # Füge die .bashrc-Datei zum Git hinzu
        git add .

        # Committe die Änderungen mit einer Nachricht
        git commit -m "Update setup"

        # Pushe die Änderungen zum remote Repository
        git push
        echo "Das directory wurde erfolgreich aktualisiert und gepusht."
    else
        echo "Das Klonen des Repositories ist fehlgeschlagen."
    fi

    # Wechsle zurück ins Home-Verzeichnis
    cd $HOME
else
